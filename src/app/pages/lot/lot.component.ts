...
export class LotComponent implements OnInit {
    ...
    this.columnDefs.forEach((column: ColDef) => {
      if (column.field === 'entry_id') {
        ...
        column.cellRendererParams = {
          inRouterLink: '/lot/edit-lot/',
          inViewLink: '/lot/view-lot/',
          action: this
        };
      }
    });
  ...
  onAddLot() {
    if (this.addLotPermission) {
      this.router.navigate(['./lot/add-lot']);
    } else {
...
@Component({
  template: `
  <i class="ft-edit-2 info font-medium-1 mr-2" (click)="editLot()"></i>
  <i class="ft-info info font-medium-1 mr-2" (click)="viewLot()"></i>
  <i class="ft-x danger font-medium-1 mr-2" (click)="onDeleteLot()"></i>`,
  styleUrls: ['./lot.component.scss']
})
