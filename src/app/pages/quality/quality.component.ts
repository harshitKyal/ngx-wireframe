...
export class QualityComponent implements OnInit {
    ...
    this.columnDefs.forEach((column: ColDef) => {
      if (column.field === 'entry_id') {
        ...
        column.cellRendererParams = {
          inRouterLink: '/quality/edit-quality/',
          action: this
        };
      }
    });
  ...
  onAddQuality() {
    if (this.addQualityPermission) {
      this.router.navigate(['./quality/add-quality']);
    } else {
...
@Component({
  template: `
  <i class="ft-edit-2 info font-medium-1 mr-2" (click)="editQuality()"></i>
  <i class="ft-x danger font-medium-1 mr-2" (click)="onDeleteQuality()"></i>`,
  styleUrls: ['./quality.component.scss']
})
